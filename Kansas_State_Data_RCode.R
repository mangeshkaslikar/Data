...
delim =","
dec = "."
csvdata_Kansas  = read.csv(file ="C:\\Users\\MangeshVilas\\Documents\\Spring2014\\DIC\\Project1\\KAN.csv", header = TRUE,sep = delim, dec = dec, stringsAsFactors=FALSE )
csvdata_Kansas$LATITUDE = as.numeric(csvdata_Kansas$LATITUDE)
csvdata_Kansas$LONGITUDE = as.numeric(csvdata_Kansas$LONGITUDE)
csvdata_Kansas$PRCP[csvdata_Kansas$PRCP ==-9999] = 0
csvdata_Kansas$PRCP[csvdata_Kansas$PRCP ==9999] = 0

# csvdata_Kansas$DATE  <- as.Date(as.character(csvdata_Kansas$DATE) ,'%Y%m%d')


#StationList =  sqldf("select distinct(STATION_NAME) from csvdata_Kansas")
#head(StationList)

# Precipitation 

 # head(csvdata_Kansas,10)
#head(StationList_Precipitation)
StationList_Precipitation  = sqldf("Select distinct(STATION_NAME) as Station_Name, 
                                   sum(PRCP) as TotalPrecipitationCount, LATITUDE as Latitude, LONGITUDE as Longitude 
                                   from csvdata_Kansas where LATITUDE <> 'unknown' and LONGITUDE <> 'unknown'
                                   group by STATION_NAME, LATITUDE, LONGITUDE")
#head(StationList_Precipitation)
# floor(StationList_Precipitation$LONGITUDE)
# General Graph of Precipitation : GGPlot 
# ggplot(StationList_Precipitation, aes(x = Station_Name, y = TotalPrecipitationCount)) + geom_histogram(stat ="identity")
...
StationList_Precipitation$Precipitation_Type_in_1000mm[TotalPrecipitationCount < 25000] <- "Mild(<2.5)"
StationList_Precipitation$Precipitation_Type_in_1000mm[TotalPrecipitationCount >= 25000 & TotalPrecipitationCount < 50000] <- "Average(2.5 to 5.0)"
StationList_Precipitation$Precipitation_Type_in_1000mm[TotalPrecipitationCount >= 50000] <- "High(>5.0)"
...
pie3D(Pie_ChartData[[2]],labels = Pie_ChartData[[1]],radius=0.8,height=0.1,theta=pi/5,
      start=1,border=par("fg"),labelcex=1.0,explode =0.2 , 
      main="Distribution of Precipitation(tenths of mm) Data Recorded \n over stations  of Kansas[in 1000mm]")
...
summary_preci <- summaryBy (TotalPrecipitationCount~Precipitation_Type, data = StationList_Precipitation, FUN = statistics)

## First Map 
map <- get_map(location = 'Kansas, United States', zoom = 7)
#ggmap(map)

#colormap <- c("Violet","Blue","Green","Yellow","Red","White")
colormap <- c("Blue","Green","Red")
Prec_Total_kansas <- ggmap(map) %+% StationList_Precipitation +
...
print(Prec_Total_kansas)
 
# Second Map : Point Map 
mapPoints <- ggmap(map) + geom_point(data = StationList_Precipitation ,aes(x = Longitude, y = Latitude, size = TotalPrecipitationCount), alpha = 0.7,colour="blue")
mapPoints
...
Sum_Map_Kansas <- get_map(location = 'Kansas, United States', zoom = 7)
#ggmap(Sum_Map_Kansas)

Temp_Data_Summer = sqldf("select STATION_NAME, LATITUDE AS Latitude, LONGITUDE as Longitude,DATE, AVG(TMAX) AS TMAX 
            from csvdata_Kansas where DATE >= 20060401 AND DATE <=20060731 
...
Summer_Map_Kansas_Total <- ggmap(Sum_Map_Kansas) %+% subset(Temp_Data_Summer,TMAX != 'NA') +
...
print(Summer_Map_Kansas_Total)


# Query For Winter Data 
Winter_Map_Kansas <- get_map(location = 'Kansas, United States', zoom = 7)
#ggmap(Winter_Map_Kansas)

Temp_Data_Winter = sqldf("select STATION_NAME, LATITUDE AS Latitude, LONGITUDE as Longitude,DATE, AVG(TMIN) AS TMIN 
            from csvdata_Kansas where DATE >= 20061101 AND DATE <=20070228 
...
Winter_Map_kansas_Total <- ggmap(Winter_Map_Kansas) %+% subset(Temp_Data_Winter,TMIN != 'NA') +
  aes(x  =Longitude,y = Latitude,z = TMIN) +
  stat_summary2d() +   scale_fill_gradientn(name = "Temperture Recorded \n (in tenths of Degree C)",
                                            colours = colormap,space = "Lab") + 
  labs(x = "Longitude",y = "Latitude") +coord_map()

print(Winter_Map_kansas_Total)
...
kclus <- kmeans(mat,5)
mat2 = data.frame(x)
map = get_map(location = 'Kansas, United States', zoom = 6)
...
Year1_Kansas = sqldf("select avg(TMAX) as TMAX, avg(TMIN) as TMIN from csvdata_Kansas where Date like '2006%'")
Year2_Kansas = sqldf("select avg(TMAX) as TMAX, avg(TMIN) as TMIN from csvdata_Kansas where Date like '2007%'")
Year3_Kansas = sqldf("select avg(TMAX) as TMAX, avg(TMIN) as TMIN from csvdata_Kansas where Date like '2008%'")
Year4_Kansas = sqldf("select avg(TMAX) as TMAX, avg(TMIN) as TMIN from csvdata_Kansas where Date like '2009%'")
Year5_Kansas = sqldf("select avg(TMAX) as TMAX, avg(TMIN) as TMIN from csvdata_Kansas where Date like '2010%'")
Year6_Kansas = sqldf("select avg(TMAX) as TMAX, avg(TMIN) as TMIN from csvdata_Kansas where Date like '2011%'")
Year7_Kansas = sqldf("select avg(TMAX) as TMAX, avg(TMIN) as TMIN from csvdata_Kansas where Date like '2012%'")

Year1_Kansas_Total <- rbind( Year1_Kansas, Year2_Kansas, Year3_Kansas, Year4_Kansas, Year5_Kansas, Year6_Kansas, Year7_Kansas)

TMAX = Year1_Kansas_Total$TMAX
TMIN = Year1_Kansas_Total$TMIN
...
barplot(Rate2, beside=TRUE, names.arg=c("2006", "2007","2008","2009","2010","2011","2012"),
        col=c("blue", "red"),border="black", main=c("Maximum and Minimum Temperatures in Kansas \n Over the Years ( 2006 - 2012)"),
...
legend("left",legend = c("Max Temp.", "Min Temp."),cex=1,fill = c("darkblue", "red"),bty = "n")

#--------------------------------
# Precipitation 

Prcp1_Kansas = sqldf("select SUM(PRCP) as PRCP from csvdata_Kansas where Date like '2006%'")
Prcp2_Kansas = sqldf("select SUM(PRCP) as PRCP  from csvdata_Kansas where Date like '2007%'")
Prcp3_Kansas = sqldf("select SUM(PRCP) as PRCP  from csvdata_Kansas where Date like '2008%'")
Prcp4_Kansas = sqldf("select SUM(PRCP) as PRCP  from csvdata_Kansas where Date like '2009%'")
Prcp5_Kansas = sqldf("select SUM(PRCP) as PRCP  from csvdata_Kansas where Date like '2010%'")
Prcp6_Kansas = sqldf("select SUM(PRCP) as PRCP  from csvdata_Kansas where Date like '2011%'")
Prcp7_Kansas = sqldf("select SUM(PRCP) as PRCP  from csvdata_Kansas where Date like '2012%'")

PRCP_Total <- rbind( Prcp1_Kansas, Prcp2_Kansas, Prcp3_Kansas, Prcp4_Kansas, Prcp5_Kansas, Prcp6_Kansas, Prcp7_Kansas)
...
barplot(Rate2, beside=TRUE, names.arg=c("2006", "2007","2008","2009","2010","2011","2012"),
        col=c("Blue"),border="black", main=c("Total Precipitation(in tenths of mm) Recorded in Kansas \n Over the Years ( 2006 - 2012)"),xlab="Year",ylab="Precipitation (in tenths of mm) ",font.lab=3)
