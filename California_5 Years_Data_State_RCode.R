...
delim =","
dec = "."
csvdata_California  = read.csv(file ="C:\\Users\\MangeshVilas\\Documents\\Spring2014\\DIC\\Project1\\CA.csv", header = TRUE,sep = delim, dec = dec, stringsAsFactors=FALSE )
csvdata_California$LATITUDE = as.numeric(csvdata_California$LATITUDE)
csvdata_California$LONGITUDE = as.numeric(csvdata_California$LONGITUDE)
csvdata_California$PRCP[csvdata_California$PRCP ==-9999] = 0
csvdata_California$PRCP[csvdata_California$PRCP ==9999] = 0

# csvdata_California$DATE  <- as.Date(as.character(csvdata_California$DATE) ,'%Y%m%d')


#StationList =  sqldf("select distinct(STATION_NAME) from csvdata_California")
#head(StationList)

# Precipitation 

# head(csvdata_California,10)
...
StationList_Precipitation  = sqldf("Select distinct(STATION_NAME) as Station_Name, 
                                   ...
                                   from csvdata_California where LATITUDE <> 'unknown' and LONGITUDE <> 'unknown'
...
pie3D(Pie_ChartData[[2]],labels = Pie_ChartData[[1]],radius=0.8,height=0.1,theta=pi/4,
      ...
      main="Distribution of Precipitation(in tenths of mm) Data Recorded \n over stations for California[in 1000mm]")
...
## First Map 
map <- get_map(location = 'California, United States', zoom = 6)
ggmap(map)

#colormap <- c("Violet","Blue","Green","Yellow","Red","White")
colormap <- c("Blue","Green","Red")
Prec_CA <- ggmap(map) %+% StationList_Precipitation +
...
Sum_Map_California <- get_map(location = 'California, United States', zoom = 6)
#ggmap(Sum_Map_California)

Temp_Data_Summer = sqldf("select STATION_NAME, LATITUDE AS Latitude, LONGITUDE as Longitude,DATE, AVG(TMAX) AS TMAX 
                         from csvdata_California where DATE >= 20060401 AND DATE <=20060731 
...
Summer_Map_California_Total <- ggmap(Sum_Map_California) %+% subset(Temp_Data_Summer,TMAX != 'NA') +
...
print(Summer_Map_California_Total)


# Query For Winter Data 
Winter_Map_California <- get_map(location = 'California, United States', zoom = 6)
#ggmap(Winter_Map_California)

Temp_Data_Winter = sqldf("select STATION_NAME, LATITUDE AS Latitude, LONGITUDE as Longitude,DATE, AVG(TMIN) AS TMIN 
                         from csvdata_California where DATE >= 20061101 AND DATE <=20070228 
...
Winter_Map_California_Total <- ggmap(Winter_Map_California) %+% subset(Temp_Data_Winter,TMIN != 'NA') +
  aes(x  =Longitude,y = Latitude,z = TMIN) +
  stat_summary2d() +   scale_fill_gradientn(name = "Temperture Recorded \n (in tenths of Degree C)",
                                            colours = colormap,space = "Lab") + 
  labs(x = "Longitude",y = "Latitude") +coord_map()

print(Winter_Map_California_Total)





# map <- get_map(location = c(lon = -94, lat = 42),
#                   source = c("osm"))

# map.in <- get_map(location = c(min(StationList_Precipitation$Longitude),  min(StationList_Precipitation$Latitude),
#                              max(StationList_Precipitation$Longitude), max(StationList_Precipitation$Latitude)),
#               source = "osm")
# ggmap(map.in)
...
Year1_California = sqldf("select avg(TMAX) as TMAX, avg(TMIN) as TMIN from csvdata_California where Date like '2006%'")
Year2_California = sqldf("select avg(TMAX) as TMAX, avg(TMIN) as TMIN from csvdata_California where Date like '2007%'")
Year3_California = sqldf("select avg(TMAX) as TMAX, avg(TMIN) as TMIN from csvdata_California where Date like '2008%'")
Year4_California = sqldf("select avg(TMAX) as TMAX, avg(TMIN) as TMIN from csvdata_California where Date like '2009%'")
Year5_California = sqldf("select avg(TMAX) as TMAX, avg(TMIN) as TMIN from csvdata_California where Date like '2010%'")
Year6_California = sqldf("select avg(TMAX) as TMAX, avg(TMIN) as TMIN from csvdata_California where Date like '2011%'")
Year7_California = sqldf("select avg(TMAX) as TMAX, avg(TMIN) as TMIN from csvdata_California where Date like '2012%'")

Year1_California_Total <- rbind( Year1_California, Year2_California, Year3_California, Year4_California, Year5_California, Year6_California, Year7_California)

TMAX = Year1_California_Total$TMAX
TMIN = Year1_California_Total$TMIN
...
barplot(Rate3, beside=TRUE, names.arg=c("2006", "2007","2008","2009","2010","2011","2012"),
        col=c("blue", "red"),border="black", main=c("Maximum and Minimum Temperatures in California \n Over the Years ( 2006 - 2012)"),
...
Prcp1_California = sqldf("select SUM(PRCP) as PRCP from csvdata_California where Date like '2006%'")
Prcp2_California = sqldf("select SUM(PRCP) as PRCP  from csvdata_California where Date like '2007%'")
Prcp3_California = sqldf("select SUM(PRCP) as PRCP  from csvdata_California where Date like '2008%'")
Prcp4_California = sqldf("select SUM(PRCP) as PRCP  from csvdata_California where Date like '2009%'")
Prcp5_California = sqldf("select SUM(PRCP) as PRCP  from csvdata_California where Date like '2010%'")
Prcp6_California = sqldf("select SUM(PRCP) as PRCP  from csvdata_California where Date like '2011%'")
Prcp7_California = sqldf("select SUM(PRCP) as PRCP  from csvdata_California where Date like '2012%'")

PRCP_Total <- rbind( Prcp1_California, Prcp2_California, Prcp3_California, Prcp4_California, Prcp5_California, Prcp6_California, Prcp7_California)
...
barplot(Rate2, beside=TRUE, names.arg=c("2006", "2007","2008","2009","2010","2011","2012"),
        col=c("Blue"),border="black", main=c("Total Precipitation(tenths of mm) Recorded in California \n Over the Years ( 2006 - 2012)"),xlab="Year",ylab="Precipitation (in tenths of mm) ",font.lab=3)
